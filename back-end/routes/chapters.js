...
/* ---------- POST /api/chapters (chỉ tác giả của novel) ---------- */
router.post("/", async (req, res) => {
  try {
    ...
    // chỉ author của novel mới được thêm chương
    if (String(novel.authorId) !== String(user._id)) {
    ...
    console.error("POST /chapters error:", e);
    res.status(500).json({ message: "Server error" });
  }
});
